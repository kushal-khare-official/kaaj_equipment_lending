...
function App() {
  ...
  const [activeTab, setActiveTab] = useState<'borrower' | 'status' | 'uw'>('borrower')
  const [applications, setApplications] = useState<any[]>([])
  const [selectedAppId, setSelectedAppId] = useState<string | null>(null)
  const [uwError, setUwError] = useState<string | null>(null)
  const [lenderEdits, setLenderEdits] = useState<Record<string, string>>({})
  ...
  const loadUwData = () => {
    listApplications().then(setApplications).catch((e) => setUwError(e.message))
    ...
    <div className="min-h-screen bg-slate-50 text-slate-900">
      <header className="border-b border-slate-200 bg-white">
        <div className="mx-auto flex max-w-5xl items-center justify-between px-6 py-4">
          ...
          <div className="text-sm text-slate-500">Borrower Experience</div>
        </div>
      </header>
      <main className="mx-auto max-w-5xl px-6 py-10 space-y-6">
        <div className="flex gap-2">
          {(['borrower', 'status', 'uw'] as const).map((tab) => (
            <button
              key={tab}
              className={`rounded px-4 py-2 text-sm font-semibold ${activeTab === tab ? 'bg-slate-900 text-white' : 'bg-white text-slate-700 border border-slate-200'
                }`}
              onClick={() => setActiveTab(tab)}
            >
              {tab === 'borrower' ? 'Borrower Form' : tab === 'status' ? 'Status & Docs' : 'Underwriter Console'}
            </button>
          ))}
        </div>
        ...
        {activeTab === 'uw' && (
          <div className="rounded-lg border border-slate-200 bg-white p-6 shadow-sm">
            <div className="text-sm font-semibold mb-2 text-slate-800">Underwriter Console (stub)</div>
            {uwError && <div className="text-sm text-red-600">{uwError}</div>}
            <div className="mt-4 grid grid-cols-1 gap-4 lg:grid-cols-2">
                    ...
                    {applications.map((a) => (
                      <li
                        ...
                        className={`flex items-center justify-between rounded border px-2 py-1 ${
                          selectedAppId === a.id ? 'border-slate-800' : 'border-slate-200'
                        }`}
                      >
                        <div>
                          <div className="font-semibold text-xs">{a.id}</div>
                        ...
                        <div className="flex gap-2">
                          ...
                          <button
                            ...
                            onClick={() =>
                              rerunMatch(a.id)
                                .then(() => setUwError(null))
                                .catch((e) => setUwError(e.message))
                            }
                          >
                            Rerun match
                          </button>
                          <button
                            className="text-xs rounded bg-amber-600 text-white px-2 py-1"
                            onClick={() =>
                              requestDoc(a.id, 'bank_statements')
                                .then(() => setUwError(null))
                                .catch((e) => setUwError(e.message))
                            }
                          >
              ...
              <div className="border border-slate-200 rounded p-4">
                <div className="text-sm font-semibold mb-2">Lenders & policy edit (name only)</div>
                {lenders.length === 0 ? (
                  <div className="text-sm text-slate-500">No lenders</div>
                ) : (
                      ...
                      <li key={l.id} className="rounded border border-slate-200 p-2">
                        <div className="flex items-center gap-2">
                          <input
                            ...
                            defaultValue={l.name}
                            onChange={(e) =>
                              setLenderEdits((prev) => ({ ...prev, [l.id]: e.target.value }))
                            }
                          />
                          <button
                            className="text-xs rounded bg-slate-900 text-white px-2 py-1"
                            onClick={() =>
                              updateLenderName(l.id, lenderEdits[l.id] ?? l.name)
                                .then(() => loadUwData())
                                .catch((e) => setUwError(e.message))
                            }
                          >
